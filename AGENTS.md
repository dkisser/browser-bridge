...
- **Bridge-core merge (ADR-0010)**: the pre-merge `apps/websocket/` and `apps/local-proxy/` are gone; both halves live in `apps/bridge-core/` (`server/inbound.ts` for the CLI-facing 3001 WebSocket, `browser-server.ts` for the extension-facing 3002 WebSocket, `mcp/` for the 3003 Streamable HTTP MCP server, `router.ts` for the in-process command dispatch). The `@browser-bridge/websocket` package is gone — its `protocol` and `client` modules are in-tree under `apps/bridge-core/src/`.
