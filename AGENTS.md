...
- **Branch `go-rewrite`**: `apps/bridge-core/` is now a Go module (`go.mod`, `cmd/bridge-core`, `internal/`) replacing the deleted Bun/TS implementation — see `docs/adr/0012-control-plane-and-cli-in-go.md`. `apps/cli/` stays TS/Bun as the spike's test driver with the WS `protocol`/`client` vendored into its `src/`; a Go `cmd/bridge` replaces it in Phase 2. The TS bridge-core remains available in a `main` worktree for fixture extraction.
- **Bridge-core merge (ADR-0011)**: the pre-merge `apps/websocket/` and `apps/local-proxy/` are gone; both halves lived in `apps/bridge-core/` (TS, deleted on this branch). The `@browser-bridge/websocket` package is gone — its `protocol` and `client` modules were in-tree under `apps/bridge-core/src/` and are now vendored in `apps/cli/src/`.
