...
- **Package scope mismatch is intentional**: apps use `@browser-bridge/*`, the shared package uses `@my/shared`. Do not "fix" this.
- **No build step for workspace packages**: `@my/shared` and `@browser-bridge/websocket` ship raw `.ts` source. Do not add a build step.
