...
Workflow: call `list_browsers` first to pick a browser (`set_browser` when
several are online), then `tab_list` to get a valid `tab_id`. Every
tab-scoped tool below requires `tab_id: number` — use the ids reported by
`tab_list`, never guess.
...
| `go_back` | Go back one page in browser history | `tab_id: number`, `timeout_ms?: number` |
| `go_forward` | Go forward one page in browser history | `tab_id: number`, `timeout_ms?: number` |
| `refresh` | Refresh the current page | `tab_id: number`, `timeout_ms?: number` |
| `click` | Click an element by CSS selector | `selector: string`, `tab_id: number`, `timeout_ms?: number` |
| `type` | Type text into an input | `selector: string`, `text: string`, `submit?: boolean`, `tab_id: number`, `timeout_ms?: number` |
| `select` | Select a dropdown option by value | `selector: string`, `value: string`, `tab_id: number`, `timeout_ms?: number` |
| `scroll` | Scroll the page or an element | `x: number`, `y: number`, `selector?: string` (default `page`), `tab_id: number`, `timeout_ms?: number` |
