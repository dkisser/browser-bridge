...
1. **WebSocket server 无 userId 归属校验**（M1）：`apps/bridge-core/internal/inbound/inbound.go` 的 response 分支向所有 CLI 连接广播响应，command 分支仅按 `browserId` 路由，未校验发起连接是否拥有目标 browser。需要在 server 侧建立 userId → browser 归属映射并按归属分发。
