...
import { executeTabNew } from '../../../src/mcp/tools/tab-new';
...
describe('executeTabNew', () => {
  ...
  it('opens tab in background by default', async () => {
    const server = Bun.serve({
      port: 0,
      hostname: '127.0.0.1',
      fetch(req, wsServer) {
        ...
        message(ws, data) {
          ...
          const payload =
            envelope.type === 'event'
              ...
              : {
                  status: 'ok',
                  data: { id: 101, url: 'https://example.com', active: false },
                  message: 'New tab 101 opened',
                };
          ws.send(
            encode('response', payload, {
              id: envelope.id,
