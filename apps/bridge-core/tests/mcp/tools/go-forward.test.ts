...
import { executeGoForward } from '../../../src/mcp/tools/go-forward';

describe('executeGoForward', () => {
  it('sends goForward command and returns success', async () => {
        ...
        message(ws, data) {
          ...
          const payload =
            envelope.type === 'event'
              ...
              : { status: 'ok', message: 'Went forward' };
          ws.send(
    ...
    try {
      const result = await executeGoForward(
        {
          sessionId: 's1',
          sessions,
      ...
      expect(result).toContain('Went forward');
    } finally {
      server.stop();
    }
  });
