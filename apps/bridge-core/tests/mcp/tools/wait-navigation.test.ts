...
import { executeWaitNavigation } from '../../../src/mcp/tools/wait-navigation';

describe('executeWaitNavigation', () => {
  it('sends wait:navigation command and returns success', async () => {
        ...
        message(ws, data) {
          ...
          const payload =
            envelope.type === 'event'
              ...
              : { status: 'ok', message: 'Navigation complete' };
          ws.send(
    ...
    try {
      const result = await executeWaitNavigation(
        {
          sessionId: 's1',
          sessions,
