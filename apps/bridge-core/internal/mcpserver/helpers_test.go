...
type fakeRouter struct {
	...
	script   func(c capturedCommand) (protocol.ResponsePayload, bool)
}

func (f *fakeRouter) HandleInboundCommand(envelope protocol.Envelope, sender protocol.TextSender) {
	var payload struct {
	...
	text, err := protocol.Encode(protocol.TypeResponse, raw, envelope.ID, envelope.BrowserID)
	if err != nil {
...
func (f fakeRegistry) ListBrowsers() []protocol.BrowserConnection { return f.browsers }

func onlineBrowser(id string) protocol.BrowserConnection {
	return protocol.BrowserConnection{BrowserID: id, UserID: "extension", Status: protocol.StatusOnline, LastSeen: 1}
}

// newTestServer builds a Server with the fakes and a silent logger.
func newTestServer(router *fakeRouter, browsers []protocol.BrowserConnection) *Server {
	return New(Options{
		Router:         router,
...
func newTestClient(t *testing.T, srv *Server) *mcp.ClientSession {
	t.Helper()
