...
func TestResolveBrowser(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			explicit: "b-1",
			browsers: []protocol.BrowserConnection{onlineBrowser("b-1"), onlineBrowser("b-2")},
			wantID:   "b-1",
		},
		{
		...
		},
		{
			...
			browsers: []protocol.BrowserConnection{
				onlineBrowser("b-1"),
				{BrowserID: "b-2", UserID: "extension", Status: protocol.StatusOffline, LastSeen: 1},
			},
			wantFail: `Browser "b-2" is not online (status: offline).`,
		},
		{
			name:     "implicit none online",
			browsers: []protocol.BrowserConnection{{BrowserID: "b-1", UserID: "extension", Status: protocol.StatusOffline, LastSeen: 1}},
			wantFail: "No browser connected. Start the extension/local-proxy first.",
		},
		{
			name:     "implicit single online",
			browsers: []protocol.BrowserConnection{onlineBrowser("b-1"), {BrowserID: "b-2", UserID: "extension", Status: protocol.StatusIdleWait, LastSeen: 1}},
			wantID:   "b-1",
		},
		{
...
func TestWithRecoveryHint(t *testing.T) {
	tests := []struct {
		name    string
		in      protocol.ResponsePayload
		wantMsg string
		wantErr string
	}{
		{
			name:    "ok payload untouched",
			in:      protocol.ResponsePayload{Status: "ok", Message: "fine"},
			wantMsg: "fine",
		},
		{
			name:    "structured reason appends to message",
			in:      protocol.ResponsePayload{Status: "error", Reason: "tab_not_found", Message: "gone"},
		...
		},
		{
			...
			in:      protocol.ResponsePayload{Status: "error", Reason: "injection_failed", Error: "cs_unavailable"},
		...
		},
		{
			...
			in:      protocol.ResponsePayload{Status: "error", Reason: "mystery", Error: "boom"},
			wantErr: "boom",
		},
		{
			name:    "legacy No tab with id pattern",
			in:      protocol.ResponsePayload{Status: "error", Error: "No tab with id: 42"},
		...
		},
		{
			...
			in:      protocol.ResponsePayload{Status: "error", Error: "no tab with id: 7"},
		...
		},
		{
			...
			in:      protocol.ResponsePayload{Status: "error", Error: "No tab with id: 42 — see tab_list"},
			wantErr: "No tab with id: 42 — see tab_list",
		},
		{
			name:    "hint not doubled",
			in:      protocol.ResponsePayload{Status: "error", Reason: "no_listener", Message: "dead The content script did not respond. Reload the page or retry the command."},
...
func TestCommandErrorMessage(t *testing.T) {
	tests := []struct {
		...
		{name: "message wins", in: protocol.ResponsePayload{Message: "m", Error: "e"}, fallback: "f", want: "m"},
		{name: "error next", in: protocol.ResponsePayload{Error: "e"}, fallback: "f", want: "e"},
		{name: "fallback last", in: protocol.ResponsePayload{}, fallback: "f", want: "f"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
