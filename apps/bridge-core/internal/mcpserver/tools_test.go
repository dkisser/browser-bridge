...
func TestCommandTools(t *testing.T) {
	ok := protocol.ResponsePayload{Status: "ok"}

	tests := []struct {
		name        string
		tool        string
		args        map[string]any
		respond     protocol.ResponsePayload
		wantCommand string
		...
		},
		{
			...
			tool:        "pageinfo",
			args:        map[string]any{"tab_id": 1},
			respond:     protocol.ResponsePayload{Status: "ok", Data: []byte(`{"title":"T","id":1}`)},
		...
		},
		{
			...
			tool:        "tab_list",
			args:        map[string]any{},
			respond:     protocol.ResponsePayload{Status: "ok", Data: []byte(`[{"id":1,"title":"T"}]`)},
		...
		},
		{
			...
			tool:        "get_text",
			args:        map[string]any{"selector": "#a", "tab_id": 1},
			respond:     protocol.ResponsePayload{Status: "ok", Data: []byte(`{"text":"hello"}`)},
			wantCommand: "gettext",
			wantParams:  map[string]any{"selector": "#a", "tabId": float64(1)},
			wantText:    "hello",
		},
		{
			...
			tool:        "get_html",
			args:        map[string]any{"selector": "#a", "tab_id": 1},
			respond:     protocol.ResponsePayload{Status: "ok", Data: []byte(`{"html":"<p>x</p>"}`)},
			wantCommand: "gethtml",
			wantParams:  map[string]any{"selector": "#a", "tabId": float64(1)},
			wantText:    "<p>x</p>",
		},
		{
			...
			tool:        "snapshot",
			args:        map[string]any{"tab_id": 1},
			respond:     protocol.ResponsePayload{Status: "ok", Data: []byte(`{"snapshot":"snap","truncated":false,"nodes_total":5,"nodes_emitted":3,"tier":1}`)},
		...
		},
		{
			...
			respond:     protocol.ResponsePayload{Status: "ok", Data: []byte(`{"snapshot":"snap","truncated":true,"nodes_total":9,"nodes_emitted":4,"tier":2}`)},
			wantCommand: "snapshot",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			router := &fakeRouter{script: func(capturedCommand) (protocol.ResponsePayload, bool) {
				return tt.respond, true
			}}
			session := newTestClient(t, newTestServer(router, []protocol.BrowserConnection{onlineBrowser("b-1")}))
...
func TestMessagePreferredOverFallback(t *testing.T) {
	router := &fakeRouter{script: func(capturedCommand) (protocol.ResponsePayload, bool) {
		return protocol.ResponsePayload{Status: "ok", Message: "Navigated!"}, true
	}}
	session := newTestClient(t, newTestServer(router, []protocol.BrowserConnection{onlineBrowser("b-1")}))
...
func TestCommandErrorMapping(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "error code",
			respond:  protocol.ResponsePayload{Status: "error", Error: "boom"},
			wantText: "Tool 'click' execution failed: boom",
		},
		{
			name:     "message beats error",
			respond:  protocol.ResponsePayload{Status: "error", Error: "code", Message: "nice message"},
			wantText: "Tool 'click' execution failed: nice message",
		},
		{
			name:     "structured reason appends its recovery hint to message",
			respond:  protocol.ResponsePayload{Status: "error", Reason: "restricted_page", Message: "nope"},
		...
		},
		{
			...
			respond:  protocol.ResponsePayload{Status: "error", Reason: "no_listener", Error: "content_script_unavailable"},
		...
		},
		{
			...
			respond:  protocol.ResponsePayload{Status: "error", Error: "No tab with id: 42"},
		...
		},
		{
			...
			respond:  protocol.ResponsePayload{Status: "error", Error: "No tab with id: 42 — call tab_list"},
...
func TestCommandTimeout(t *testing.T) {
	router := &fakeRouter{script: func(capturedCommand) (protocol.ResponsePayload, bool) {
		return protocol.ResponsePayload{}, false // never respond
	}}
	srv := newTestServer(router, []protocol.BrowserConnection{onlineBrowser("b-1")})
...
func TestBrowserResolutionErrors(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			browsers: []protocol.BrowserConnection{onlineBrowser("b-1")},
			pin:      "b-9",
		...
		},
		{
			...
			browsers: []protocol.BrowserConnection{
				onlineBrowser("b-1"),
				{BrowserID: "b-2", UserID: "extension", Status: protocol.StatusIdleWait, LastSeen: 1},
			},
			pin:      "b-2",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			router := &fakeRouter{script: func(capturedCommand) (protocol.ResponsePayload, bool) {
				return protocol.ResponsePayload{Status: "ok"}, true
			}}
...
func TestSetBrowserPinsSession(t *testing.T) {
	...
	srv := newTestServer(router, []protocol.BrowserConnection{onlineBrowser("b-1"), onlineBrowser("b-2")})
	session := newTestClient(t, srv)
...
func TestListBrowsers(t *testing.T) {
	tests := []struct {
		...
		{name: "none", browsers: nil, wantText: "No browsers connected."},
		{
			...
			browsers: []protocol.BrowserConnection{
				onlineBrowser("b-1"),
				{BrowserID: "b-2", UserID: "extension", Status: protocol.StatusOffline, LastSeen: 2},
			},
			wantText: "- b-1 (online)\n- b-2 (offline)",
		},
	}
...
func TestGetTextGuards(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			router := &fakeRouter{script: func(capturedCommand) (protocol.ResponsePayload, bool) {
				return protocol.ResponsePayload{Status: "ok", Data: []byte(tt.data)}, true
			}}
			session := newTestClient(t, newTestServer(router, []protocol.BrowserConnection{onlineBrowser("b-1")}))
