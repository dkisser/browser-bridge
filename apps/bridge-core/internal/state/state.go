// Package state is the Go port of src/state.ts from the deleted Bun
// implementation: the on-disk bridge config (browserId + extension token
// hash) plus the in-memory browser status and the 5-second command buffer
// that rides out extension service-worker reconnects.
package state
...
func WithBufferTimeout(d time.Duration) Option {
	return func(m *Manager) { m.bufferTimeout = d }
}

// Manager holds the bridge config and browser status. All methods are safe
// for concurrent use (the TS original relied on the single-threaded event
// loop; the Go servers call in from many goroutines).
type Manager struct {
	mu            sync.Mutex
	dir           string
	config        fileConfig
...
// NewManager loads (or initializes) the config under $BB_HOME, falling back
// to ~/.browser-bridge. A load failure (missing file, bad JSON, missing
...
func (m *Manager) BrowserID() string {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.config.BrowserID
}

func (m *Manager) ExtensionTokenHash() string {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.config.ExtensionTokenHash
}

func (m *Manager) SetExtensionTokenHash(hash string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Manager) ClearExtensionTokenHash() error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Manager) Status() protocol.BrowserStatus {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.status
}
...
// in router.ts).
func (m *Manager) SetStatus(status protocol.BrowserStatus) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.status = status
	if status != protocol.StatusIdleWait && m.buffered != nil {
...
func (m *Manager) CanAcceptCommand() bool {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.status == protocol.StatusOnline || m.status == protocol.StatusIdleWait
}
...
func (m *Manager) BufferCommand(envelope string, onTimeout func()) bool {
	m.mu.Lock()
	defer m.mu.Unlock()
	if m.status != protocol.StatusIdleWait {
		return false
	}
	if m.buffered != nil {
		return false
...
func (m *Manager) GetBufferedCommand() (string, bool) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if m.buffered == nil {
		return "", false
