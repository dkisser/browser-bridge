...
func (f *fakeRegistry) SetStatus(browserID string, status protocol.BrowserStatus) bool {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func makeRouter(t *testing.T, stateOpts ...state.Option) (*Router, *state.Manager, *fakeBrowser, *fakeRegistry) {
	t.Helper()
	...
	browser := &fakeBrowser{}
	reg := &fakeRegistry{}
	return New(st, browser, reg, log.Default()), st, browser, reg
}

func commandEnvelope(browserID, id string) protocol.Envelope {
...
func decodePayload(t *testing.T, text string) protocol.ResponsePayload {
	t.Helper()
	env, err := protocol.Decode(text)
	if err != nil {
...
func TestBuffersWhileExtensionBetweenReconnects(t *testing.T) {
	...
	st.SetStatus(protocol.StatusIdleWait)
	sender := &fakeSender{}
...
func TestConnectConsumesBufferAndFlipsOnline(t *testing.T) {
	...
	if st.Status() != protocol.StatusOnline {
...
func TestDisconnectReturnsToIdleWait(t *testing.T) {
	r, st, _, reg := makeRouter(t)
	st.SetStatus(protocol.StatusOnline)
	reg.SetStatus(st.BrowserID(), protocol.StatusOnline)

	r.HandleBrowserDisconnect()

	if st.Status() != protocol.StatusIdleWait {
...
func TestForwardsToExtensionWhenConnected(t *testing.T) {
	...
	env, err := protocol.Decode(got[0])
	if err != nil {
		t.Fatal(err)
	}
	if env.ID != "c1" || env.Type != protocol.TypeCommand || env.BrowserID != st.BrowserID() {
...
func TestBufferedCommandTimesOutIntoSWTimeout(t *testing.T) {
	r, st, _, _ := makeRouter(t, state.WithBufferTimeout(20*time.Millisecond))
	st.SetStatus(protocol.StatusIdleWait)
...
func TestResponseFanoutByID(t *testing.T) {
	...
	r.HandleBrowserResponse(protocol.Envelope{
		ID:        "c2",
	...
	if env.ID != "c2" || env.Type != protocol.TypeResponse {
...
func TestBrowserEventsUpdateRegistry(t *testing.T) {
	r, st, _, reg := makeRouter(t)
	event := func(name string) protocol.Envelope {
...
// senderFunc adapts a function to protocol.TextSender.
