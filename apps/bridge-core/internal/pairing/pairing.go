// Package pairing is the Go port of src/pairing.ts: Crockford Base32 pairing
// codes with a 5-minute TTL, a rolling 5-attempt failure window that start()
// must not reset, and SHA-256-hashed bearer tokens compared in constant time.
package pairing
...
// Manager runs the pairing handshake. Token persistence is delegated to the
// caller (state.Manager in production) exactly as the TS constructor takes
...
func NewManager(getTokenHash func() string, setTokenHash func(string) error, opts ...Option) *Manager {
	m := &Manager{
...
func (m *Manager) Start() (code string, expiresIn int) {
	m.mu.Lock()
	defer m.mu.Unlock()
	code = randomCode()
...
func (m *Manager) Confirm(code string) (ConfirmResult, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	now := m.now()
