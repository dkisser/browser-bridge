// Package wsclient is the inbound-side WebSocket client used by the CLI:
// it dials the bridge-core inbound port, sends command/event envelopes, and
// correlates responses by envelope id. Go port of the deleted
// apps/cli/src/client.ts + managedClient.ts (ADR-0012 phase 2b).
package wsclient
...
// Client is a single-use WebSocket connection. It is safe for concurrent
// use, but the CLI issues one request per process and closes.
type Client struct {
	conn   *websocket.Conn
...
func (c *Client) SendCommand(ctx context.Context, browserID string, cmd protocol.CommandPayload, timeout time.Duration) (protocol.Envelope, error) {
	return c.roundTrip(ctx, protocol.TypeCommand, cmd, browserID, timeout, "command "+cmd.Command)
}

// Request sends an envelope of any type and waits for its response, the TS
// client's request (used for the list_browsers event).
func (c *Client) Request(ctx context.Context, typ protocol.Type, payload any, browserID string, timeout time.Duration) (protocol.Envelope, error) {
	return c.roundTrip(ctx, typ, payload, browserID, timeout, string(typ))
}

func (c *Client) roundTrip(ctx context.Context, typ protocol.Type, payload any, browserID string, timeout time.Duration, timeoutSubject string) (protocol.Envelope, error) {
	raw, err := json.Marshal(payload)
	if err != nil {
		return protocol.Envelope{}, fmt.Errorf("marshal %s payload: %w", typ, err)
	}
	id := protocol.NewID()
	text, err := protocol.Encode(typ, raw, id, browserID)
	if err != nil {
	...
	if err := c.write(text); err != nil {
		c.removePending(id)
		return protocol.Envelope{}, fmt.Errorf("send %s: %w", typ, err)
	}
	...
	case <-timer.C:
		c.removePending(id)
		return protocol.Envelope{}, fmt.Errorf("timeout: no response for %s within %dms", timeoutSubject, timeout.Milliseconds())
	case <-ctx.Done():
		c.removePending(id)
		return protocol.Envelope{}, fmt.Errorf("%s: %w", timeoutSubject, ctx.Err())
	}
}

// Close shuts the connection down; pending requests fail with
// ErrConnectionClosed via the read loop. Like wsconn.Close it is idempotent
...
func (c *Client) readLoop() {
	for {
		...
		if err != nil || env.Type != protocol.TypeResponse {
			continue
		}
		c.mu.Lock()
