...
func TestSetStatusCreatesEntryLazily(t *testing.T) {
	...
	b := browsers[0]
	if b.BrowserID != "b-1" || b.UserID != "extension" || b.Status != StatusOnline || b.LastSeen == 0 {
		t.Fatalf("entry = %+v", b)
	}
}
...
func TestSetStatusUpdatesExistingEntry(t *testing.T) {
	r := NewRegistry()
	r.SetStatus("b-1", StatusOnline)
	r.SetStatus("b-1", StatusOffline)
