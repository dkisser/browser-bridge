...
| 部分 | 组件 | 职责 |
|------|------|------|
| **Cloud** | CLI / Agent | 统一的浏览器控制接口，既接收用户命令，也供任意 Agent 调用。 |
...
- **Agent / 自动化系统**：把 CLI 当作标准入口，让 LLM、脚本、调度任务或其他 Agent 调用，完成需要浏览器的复杂工作流。
...
```
安装 Extension  ──▶  完成认证  ──▶  人或 Agent 在云端操作浏览器
```
...
3. **操控**：认证通过后，用户或 Agent 即可在云端通过 CLI 向浏览器发送指令。对 Agent 而言，CLI 就是控制浏览器的标准 API。
